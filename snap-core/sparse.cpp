...
void TNSparseNet::GetKeyFreq(THash<TStr, TPair<TInt, TInt> >& Freq) const {
  ...
  while(!NodeHI.IsEnd()) {
    ...
    for (int i = 0; i < Node.StrAttrIds.Len(); i++) {
      ...
      if (Node.StrAttrs[i] == TStr::GetNullStr()) {
...
int TNSparseNet::GetAttrType(TStr &attr) {
  TInt AttrId = GetAttrIdN(attr, StrType);
  int type = -1;
  THash<TInt, TNode>::TIter NodeHI = NodeH.BegI();
  while(!NodeHI.IsEnd()) {
...
int TNSparseNet::ConvertStrAttr(TStr &attr, int type, bool doCheck) {
  ...
  while(!NodeHI.IsEnd()) {
    ...
    if (index != -1) {
      TStr Val = Node.GetStrAttr(AttrId, index);
      if (Val != TStr::GetNullStr()) {
        Node.DelStrAttr(AttrId);
        if (type == IntType) {
         TInt IVal = Val.GetInt();
          Node.AddIntAttr(NewAttrId, IVal);
        } else if (type == FltType) {
          TFlt FVal = Val.GetFlt();
          Node.AddFltAttr(NewAttrId, FVal);
        }
      }
    }
    NodeHI++;
...
void TNSparseNet::GetAttrTypes(THash<TStr, TInt> &Types) {
...
void TNSparseNet::ConvertToTSV(FILE *F, int num_attrs) {
  ...
  while(!NodeHI.IsEnd()) {
    ...
    for(int i = 0; i < AttrIds.Len(); i++) {
      TNode& Node = NodeHI.GetDat();
      int index = Node.GetStrAttrIndex(TInt(i));
      if (index == -1) {
        fprintf(F, "\t%s", NULL_STR.CStr());
      } else {
        fprintf(F, "\t%s", Node.GetStrAttr(TInt(i), index).CStr());
      }
    }
    fprintf(F, "\n");
    NodeHI++;
