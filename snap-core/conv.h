...
void AddAttrTable(PTable Table, PGraph& Graph, const TStr& SrcCol, const TStr& DstCol, 
  TStrV& SrcAttrV, TStrV& DstAttrV, TStrV& EdgeAttrV, TAttrAggr AggrPolicy, TIntV& Perm,
  TInt DefaultInt, TFlt DefaultFlt, TStr DefaultStr)
{
  ...
  for (int CurrRowIdx = 0; CurrRowIdx < (Table->Next).Len(); CurrRowIdx++) {
    ...
    for (TInt i = 0; i < EdgeAttrV.Len(); i++) {
      ...
      TAttrType T = Table->GetColType(ColName);
      TInt Index = Table->GetColIdx(ColName);
      TInt EId = Perm[CurrRowIdx];
      switch (T) {
        case atInt:
          if (Table->IntCols[Index][CurrRowIdx] != DefaultInt) {
            Graph->AddIntAttrDatE(EId, Table->IntCols[Index][CurrRowIdx], ColName);
          }
          break;
        case atFlt:
          if (Table->FltCols[Index][CurrRowIdx] != DefaultFlt) {
            Graph->AddFltAttrDatE(EId, Table->FltCols[Index][CurrRowIdx], ColName);
          }
          break;
        case atStr:
          if (Table->GetStrVal(Index, CurrRowIdx) != DefaultStr) {
            Graph->AddStrAttrDatE(EId, Table->GetStrVal(Index, CurrRowIdx), ColName);
          }
          break;
      }
    }
